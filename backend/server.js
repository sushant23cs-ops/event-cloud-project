...
app.post("/api/events", auth, adminOnly, async (req, res) => {
  try {
    const { title, category, location, event_date, description, capacity } = req.body || {};
    if (!title || !category || !location || !event_date) {
      return res.status(400).json({ error: "Missing fields" });
    }

    await pool.query(
      "INSERT INTO events (title, category, location, event_date, description, capacity) VALUES (?, ?, ?, ?, ?, ?)",
      [title, category, location, event_date, description || "", capacity || 50]
    );
